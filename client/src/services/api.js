...
export const quotationsAPI = {
  ...
  downloadPDF: (id) => api.get(`/quotations/${id}/pdf`, { responseType: 'blob' }).then(r => ({ blob: r.data, filename: (r.headers['content-disposition']?.match(/filename="?([^"]+)"?/)?.[1]) || null })),
...
export const invoicesAPI = {
  ...
  downloadPDF: (id) => api.get(`/invoices/${id}/pdf`, { responseType: 'blob' }).then(r => ({ blob: r.data, filename: (r.headers['content-disposition']?.match(/filename="?([^"]+)"?/)?.[1]) || null })),
...
export const contentAPI = {
  ...
  getPipeline: (params) => api.get('/content/pipeline', { params }),
