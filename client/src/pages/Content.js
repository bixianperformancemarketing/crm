...
const STATUS_COLORS = { 'Overdue': '#ef4444', 'To Do Today': '#0ea5e9', 'In Progress': '#a78bfa', 'Review': '#f59e0b', 'Approved': '#22c55e', 'Not Approved': '#ef4444' };
const CHIP_CLASS = { 'Overdue': 'status-not-approved', 'To Do Today': 'status-todo-today', 'In Progress': 'status-in-progress', 'Review': 'status-review', 'Approved': 'status-approved', 'Not Approved': 'status-not-approved' };
      ...
      {view === 'list' && (
        <>
          <div className="content-filter-bar" style={{ marginBottom: 16 }}>
            {['', 'Overdue', 'To Do Today', 'In Progress', 'Review', 'Approved', 'Not Approved'].map((s) => (
        ...
        <div className="modal-overlay" onClick={() => setShowTask(null)}>
          <div className="modal" style={{ maxWidth: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
              {['Overdue', 'To Do Today', 'In Progress', 'Review', 'Approved', 'Not Approved'].filter(s => s !== showTask.status).map(s => (
